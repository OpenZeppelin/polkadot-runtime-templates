# Optimization Notes
> Only implement once the implementation is stable and secure. Premature optimization is bad.
...
Exploit fixed bases
Your Pedersen generators (G, H, H_i…) and any fixed basepoints should use fixed-base scalarmul with precomputed tables.
Generate wNAF tables at build time and ship them as const arrays; never rebuild at runtime.
Reduce proof size / work
Aggregated range proofs (e.g., prove 64×[0,2^k) in one proof) drastically reduce per-proof verifier work.
...
Bench & feature flags (actionable)
Create a microbenchmark crate that runs in no_std (with a std harness for local) to test:
dalek backend: u32 vs fiat
SIMD on/off
MSM window sizes
Batch sizes: 1, 4, 8, 16, 32
Aggregated ranges: 1×64, 8×32, 16×32, etc.
Pin the best combo via cargo features in both prover and verifier to keep them in lockstep.
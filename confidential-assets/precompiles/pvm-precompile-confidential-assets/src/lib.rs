//! PolkaVM Precompile for Confidential Assets
// TODO: implement OnConfidentialTransferReceived Callback and prevent reentrancy
use alloc::vec::Vec;
...
// fix import from pallet once pallet is complete
    ...
    Runtime: pallet_revive::Config + PalletConfig + frame_system::Config,
{
    ...
    fn call(
        ...
        match input {
            ...
            // -----------------------------------------------------------------
            IConfidentialAssetsCalls::confidentialMint(
                IConfidentialAssets::confidentialMintCall { asset, to, amount },
            ) => {
                let _ =
                    env.charge(<Runtime as PalletConfig>::WeightInfo::confidential_transfer())?;
            ...
            // NOTE: Pallet requires Root origin.
            // -----------------------------------------------------------------
            IConfidentialAssetsCalls::confidentialBurn(
                IConfidentialAssets::confidentialBurnCall {
                    asset,
                    from,
                    amount,
                },
            ...
            // -----------------------------------------------------------------
            IConfidentialAssetsCalls::requestDecryption(
                IConfidentialAssets::requestDecryptionCall { encrypted },
            ) => {
                let _ = env.charge(<Runtime as PalletConfig>::WeightInfo::request_decryption())?;

                ensure_non_empty(encrypted, "encrypted (cipher)")?;
                let cipher: Cipher = encrypted
                    .as_slice()
                    .try_into()
                    .map_err(|_| Error::Revert("Cipher must be exactly 32 bytes".into()))?;

                // We let the pallet manage request id & event emission.
                pallet::Pallet::<Runtime>::request_decryption(frame_origin, cipher)
                    .map(|_| Vec::new())
            ...
            IConfidentialAssetsCalls::decrypt(IConfidentialAssets::decryptCall {
                ...
                result
                    .map(|_| Vec::new())
                    .map_err(|e| revert(&e, "decrypt failed"))
            }
        }
    }
}

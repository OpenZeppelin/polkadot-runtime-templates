...
[NOTE]
====
At this point, you are free to use the omni-node but if you wish to customize the node then the node included in each template offers a suitable starting point for such customization. The command for using omni-node takes the form: `polkadot-omni-node --chain <chain_spec.json>`. For more information about using omni-node see the relevant link:https://docs.polkadot.com/develop/toolkit/parachains/polkadot-omni-node/[Polkadot Docs]
====

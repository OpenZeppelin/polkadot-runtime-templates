...
Payment: `DepositBase` will be reserved if this is the first approval, plus `threshold` times `DepositFactor`. It is returned once this dispatch happens or is canceled.
...
* `MultisigCancelled(cancelling, timepoint, multisig, call_hash)` -- when multisig call is cancelled. This also gives information on who canceled (`cancelling`), the `timepoint` of the call, id of the multisig call (`multisig`), hash of the multisig call (`call_hash`).
...
* `m-out-of-n multisig account` example:
    ...
    ** *Niche Details:*
        ...
        *** so, if the last signatory calls `approve_as_multi`, and if succeeds, the multisig operation is stuck (has to be canceled and re-created).
